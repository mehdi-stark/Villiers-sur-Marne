...
//   node scripts/capturer.mjs --base http://localhost:3000 [--pages /,/pilotage/cadrage] [--viewport 390x844] [--forger admin@x.y] [--out ./captures]
...
const ctx = await browser.newContext({ viewport: { width: vw, height: vh }, deviceScaleFactor: 2, locale: "fr-FR", timezoneId: "Europe/Paris" });
...
for (const p of PAGES) {
  ...
  await page.screenshot({ path: `${OUT}/${nom}-${vw}.png`, fullPage: true });
