...
export function Coquille({ children, commune, email, aTraiter, aPointer, demo, presentation }: { children: React.ReactNode; commune: { nom: string; courte: string; initiale: string; telephone: string; logoUrl: string | null; mentionLogo: string | null }; email: string | null; aTraiter: number; aPointer: number; demo: boolean; presentation: boolean }) {
  ...
  return <CoquilleAdmin marque={{ nom: `Agents — ${commune.nom}`, courte: "Agents", initiale: commune.initiale, sousTitre: commune.courte, logoUrl: commune.logoUrl }} sections={sections} profil={profil}><VerrouBiometrique cle="agents-passkey" />{demo && <BandeauDemo presentation={presentation} detail="File du jour, familles et démarches fictives." />}{children}</CoquilleAdmin>;
}
