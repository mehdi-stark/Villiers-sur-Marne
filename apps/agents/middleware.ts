...
const auth = creerAuth({ app: "agents", cookie: "agents_session", secretEnv: "AGENTS_AUTH_SECRET", autorise: () => true });
...
export async function middleware(req: NextRequest) {
  ...
  if (pathname === "/api/sante" || pathname === "/connexion" || pathname.startsWith("/api/auth") || pathname === "/presentation" || pathname.startsWith("/api/passkey")) return NextResponse.next();
