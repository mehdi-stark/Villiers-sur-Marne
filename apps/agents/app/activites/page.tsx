...
export default async function Activites() {
  ...
  return (
    <>
      <div className="page-tete"><div><h1>Activités et tarifs</h1><p className="muted">Grille 2025-2026 et délais de prévenance — chaque règle porte sa source.</p></div></div>
      <section className="carte">
        <div className="doc"><div className="tableau-defile"><table style={{ minWidth: 900 }}>
          ...
          <tbody>{activites.map((x) => (
            <tr key={x.id}><td><strong>{x.libelle}</strong>{x.forfaitMensuel && <div className="tiny">forfait dès {x.forfaitMensuel.declencheA}</div>}</td><td>{x.horaires}</td><td>{x.public}</td><td>{x.joursServis.map((j) => ["", "L", "Ma", "Me", "J", "V"][j]).join(" ")}</td>{x.tarifsParTranche.map((m, i) => <td key={i}>{euros(m)}</td>)}<td className="tiny">{x.prevenance.joursAvant ? `${x.prevenance.joursAvant} j ${x.prevenance.type} · ${x.prevenance.heureLimite}` : "sans réservation"}<div>{x.prevenance.source}</div></td></tr>
          ))}</tbody>
        </table></div></div>
      </section>
    </>
