...
export async function POST(req: NextRequest) {
  ...
  if (b.action === "envoyer") {
    ...
    const verdict = await garderDemandeOtp({ app: "agents", email, ip: ipDe(req), autorise: (e) => auth.emailAutorise(e) });
    if (!verdict.passe) {
      ...
      if (verdict.motif === "quota_ip" && alerterMartelement("agents")) {
        await poserAlerte("warn", "connexion_martelee_agents", "Tentatives de connexion en rafale bloquées avant la base", { ip: ipDe(req) });
      }
  ...
  if (b.action === "valider") {
    ...
    if (!formatOk || !quota(`valider|agents|${ipDe(req)}`, 20, 10 * 60_000).ok) return NextResponse.json({ ok: false }, { status: 429 });
