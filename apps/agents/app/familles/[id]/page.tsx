...
import { enfantsRattaches } from "@ville/core/donnees/enfants";
      ...
      <section className="carte pile">
        ...
        {enfants.length === 0 ? (
          ...
          <div className="pile" style={{ gap: 6 }}>
            {enfants.map((e) => (
              <LigneEnfant key={e.id} id={e.id} familleId={id} prenom={e.prenom} detail={`${fmtNaissance.format(new Date(`${e.naissance}T12:00:00Z`))} · ${age(e.naissance)} ans · ${e.ecole} · ${e.classe}`}
