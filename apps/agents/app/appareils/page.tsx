  ...
  return (
    <>
      ...
      <section className="carte pile">
        ...
        {journal.length === 0 ? <p className="petit t-2">Aucun événement.</p> : (
          <div className="file">{journal.map((j) => <div key={j.id} className="file-ligne"><div><strong>{LIB[j.evenement] ?? j.evenement}</strong><div className="mini t-3">{(j.detail as { via?: string; appareil?: string; cause?: string } | null)?.via ?? ""} {(j.detail as { appareil?: string } | null)?.appareil ?? ""} {(j.detail as { cause?: string } | null)?.cause ?? ""}</div></div><span className="mini t-3">{fmt.format(j.creeLe)} (Paris)</span></div>)}</div>
        )}
      </section>
    </>
  );
