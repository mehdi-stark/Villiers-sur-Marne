...
export async function middleware(req: NextRequest) {
  ...
  if (pathname === "/api/sante" || pathname.startsWith("/api/cron") || pathname === "/connexion" || pathname.startsWith("/api/auth") || pathname === "/api/agent" || pathname.startsWith("/api/passkey")) return NextResponse.next();
