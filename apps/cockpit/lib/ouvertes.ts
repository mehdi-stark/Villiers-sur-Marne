...
export async function toutesLesDecisions(): Promise<Ouverte[]> {
  const [cadrageMd, backlogMd, pc, pm, pb] = await Promise.all([lireDoc("CADRAGE.md"), lireDoc("BACKLOG.md"), decisionsPrises("cadrage"), decisionsPrises("marche"), decisionsPrises("backlog")]);
  const a = analyse();
  const liste: Ouverte[] = [];
  ...
  liste.push({ sujet: "marche", cle: "marche:verdict", titre: `Verdict marché ${a.final} (${a.score}/100) — que fait-on ?`, texte: "Parer la faille interop remonte le verdict à GO conditionnel sans changer les notes.", options: ["Parer la faille : je porte l'interop (contact Infocom'94 / mairie)", "Démonstrateur sans vente, risque assumé", "NO-GO accepté : on s'arrête"], recommandation: "Parer la faille : je porte l'interop (contact Infocom'94 / mairie)", bloque: "l'architecture et tout code métier", rang: 2, prise: pm.get("marche:verdict") ?? null, decideeDoc: null });
  for (const l of extraireBacklog(backlogMd)) liste.push({ sujet: "backlog", cle: l.cle, titre: l.item, texte: `Débloque : ${l.debloque} · taille ${l.taille} · doc : ${l.decisionDoc}`, options: OPTIONS_BACKLOG, recommandation: null, bloque: l.debloque, rang: 3, prise: pb.get(l.cle) ?? null, decideeDoc: backlogOuvert(l) ? null : l.decisionDoc });
