...
export function Coquille({ children, commune, famille, email, demarchesActives }: { children: React.ReactNode; commune: { nom: string; courte: string; initiale: string; telephone: string; logoUrl: string | null; mentionLogo: string | null }; famille: string | null; email: string | null; demarchesActives: number }) {
  ...
  return <CoquilleClient marque={{ nom: `Portail Famille — ${commune.nom}`, courte: "Famille", initiale: commune.initiale, sousTitre: commune.courte, logoUrl: commune.logoUrl }} destinations={destinations} profil={profil}><VerrouBiometrique cle="famille-passkey" />{children}</CoquilleClient>;
}
