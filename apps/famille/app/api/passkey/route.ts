...
function cfg(req: NextRequest): ConfigPasskeys {
  ...
  return { app: auth.app, rpName: "Portail Famille", rpId: process.env.PASSKEY_RP_ID ?? new URL(origine).hostname, origine };
}
async function session() { const s = await auth.verifierSession((await cookies()).get(auth.COOKIE)?.value); return s; }
