      ...
      <div className="semaine">
        {enfants.map((e) => (
          <section key={e.id} className="carte enfant-carte">
            <div className="enfant-tete"><span className="avatar" aria-hidden>{e.prenom.slice(0, 1)}</span><div><strong>{e.prenom}</strong><div className="mini t-3">né(e) le {new Intl.DateTimeFormat("fr-FR", { dateStyle: "long", timeZone: "Europe/Paris" }).format(new Date(e.naissance))}</div></div></div>
            <div className="backlog-meta"><span><b>École</b>{e.ecole}</span><span><b>Classe</b>{e.classe}</span></div>
          </section>
        ))}
      </div>
      <p className="mini t-3">Dossier, pièces et démarches (inscription, changement d'école, PAI) : prochains maillons.</p>
    </>
  );
}
