...
export async function GET(req: NextRequest) {
  ...
  await db.insert(schema.journalConnexions).values({ app: "famille", email, evenement: "connexion", detail: { via: "presentation", agent: req.headers.get("user-agent")?.slice(0, 160) } });
  ...
  c.set("famille_presentation", "1", { httpOnly: false, secure: process.env.NODE_ENV === "production", sameSite: "lax", maxAge: 2 * 3600, path: "/" });
