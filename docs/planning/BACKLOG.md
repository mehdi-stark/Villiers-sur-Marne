...
| Parer la faille interop par le code : adaptateur `SourceDonnees` (fictif · export Agora+ · API) avec un jeu Villiers réaliste (proposé 04/09/2026, Feature) | La démo tourne sur des données crédibles et la bascule vers Agora+ ne touche pas le front | ~1 j | En attente — après le tap « Parer la faille » sur le verdict marché |
| Mode sombre vérifié en capture (`prefers-color-scheme: dark` dans capturer.mjs) (proposé 04/09/2026, Design) | Les jetons sombres sont prouvés, pas supposés (leçon 50 : boutons invisibles) | ~1 h | En attente |
| Accusé « reportée » visible depuis le cockpit : `pnpm decisions --reporter` déclenche un push « décision X reportée dans le doc » (proposé 04/09/2026, UX) | L'opérateur voit la boucle se fermer sans rouvrir l'écran | ~1 h | En attente |
